...
import { createUser, getUserByEmailForLogin } from "../data-access/users-db/users";
...
//login for mobile
export const login = async (email: string, password: string, ip: string, deviceHash: string) => {
    ...
    if(!isWithinSlidingWindowLog( ipKey, RATE_LIMITS.SWL_IP_LIMIT, RATE_LIMITS.SWL_WINDOW )) throw new Error("RateLimitError");
    if(!isWithinSlidingWindowLog( deviceKey, RATE_LIMITS.SWL_DEVICE_LIMIT, RATE_LIMITS.SWL_WINDOW )) throw new Error("RateLimitError");
    if(!isWithinSlidingWindowLog( globalKey, RATE_LIMITS.GLOBAL_LOGIN_LIMIT, RATE_LIMITS.GLOBAL_LOGIN_WINDOW )) throw new Error("RateLimitError");
    ...
    if(!isWithinSlidingWindowLog(emailKey, RATE_LIMITS.SWL_EMAIL_LIMIT, RATE_LIMITS.SWL_WINDOW)) throw new Error("RateLimitError");
...
export const register = async (fullname: string, phoneNumber: string, email: string, password: string, ip: string, deviceHash: string) => {
    //keys
    ...
    if(!isWithinSlidingWindowLog( ipKey, RATE_LIMITS.SWL_IP_LIMIT, RATE_LIMITS.SWL_WINDOW )) throw new Error("RateLimitError");
    if(!isWithinSlidingWindowLog( deviceKey, RATE_LIMITS.SWL_DEVICE_LIMIT, RATE_LIMITS.SWL_WINDOW )) throw new Error("RateLimitError");
    if(!isWithinSlidingWindowLog( globalKey, RATE_LIMITS.GLOBAL_REGISTER_LIMIT, RATE_LIMITS.GLOBAL_REGISTER_WINDOW )) throw new Error("RateLimitError");
    ...
    if(
        !isWithinSlidingWindowLog( emailKey, RATE_LIMITS.SWL_EMAIL_LIMIT, RATE_LIMITS.SWL_WINDOW) ||
        !isWithinSlidingWindowLog( phoneKey, RATE_LIMITS.SWL_PHONE_NUM_LIMIT, RATE_LIMITS.SWL_WINDOW)
